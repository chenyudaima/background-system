...
public class ThreadPoolUtil {
    ...
    private static final ThreadPoolExecutor executor = ThreadPoolConfig.getDefaultExecutor();
    ...
    /**
     * 线程池执行任务 （不带返回值，不会抛异常，异常需要从Future获取）
     * @param runnable 任务
     * @return 线程执行状态 （用于判断线程执行是否出现异常）
     */
    ...
    /**
     * 线程池执行任务（带返回值，不会抛异常，异常需要从Future获取，不能用Lambda表达式）
     * @param callable 任务
     * @return 线程执行状态及执行结果
     */
